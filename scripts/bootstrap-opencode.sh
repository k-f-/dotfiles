...
ensure_repo() {
    ...
    if [[ -d "${dir}/.git" ]]; then
        if [[ "${UPDATE_MODE}" == "true" ]]; then
            if run_with_status "${name}: git pull" \
                bash -c 'cd "${1}" && git pull --ff-only' _ "${dir}"; then
                tick
            else
        ...
        if run_with_status "${name}: cloning" git clone "${ssh_url}" "${dir}"; then
            tick
        else
            local https_url="${ssh_url/git@github.com:/https:\/\/github.com\/}"
            if run_with_status "${name}: cloning (HTTPS)" git clone "${https_url}" "${dir}"; then
                tick
            else
...
setup_agr_mcp() {
    ...
    if run_with_status "agr-mcp: uv sync" \
        bash -c 'cd "${1}" && uv sync' _ "${CODE_DIR}/agr-mcp"; then
        tick
    else
...
setup_agentic_standards() {
    ...
    if run_with_status "agentic-dev-standards: npm install" \
        bash -c 'cd "${1}" && npm install --production' _ "${dir}"; then
        tick
    else
...
setup_opencode_plugins() {
    ...
    if run_with_status "opencode plugins: bun install" \
        bash -c 'cd "${1}" && bun install --frozen-lockfile 2>/dev/null || bun install' _ "${OPENCODE_CONFIG}"; then
        tick
    else
